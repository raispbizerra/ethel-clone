...
class UserDao():
	...
	def create_user(self, user : User):
		...
		try:
			...
			self.c.conn.execute(sql, [user.get_name(), user.get_username(), user.get_password(), user.get_email(), user.get_is_adm()])
			self.c.conn.commit()
			result = True
		except:
			print ("Error!")
	...
	def read_user(self, usr_username : str, usr_password : str):
		...
		try:
			...
			if result:
				user = User(result[0], result[1], usr_username, usr_password, result[2], result[3])
		except:
			print ("Error!")
		finally:
			self.c.close()
	...
	def update_user(self, user : User):
		...
		try:
			self.c.connect(self.db)
			self.c.conn.execute(sql, [user.get_name(), user.get_username(), user.get_password(), user.get_email(), user.get_is_adm(), user.get_cod()])
			self.c.conn.commit()
			result = True
		except:
			print ("Error!")
