	...
	def read_exam(self, ex_cod : int):
		...
		try:
			...
			if result:
				...
				exam = DynamicExam(ex_cod, cop_x, cop_y, date, result[3], result[4])
		except:
			print ("Error!")
		finally:
			self.c.close()
		...
		try:
			...
			if results:
				for result in results:
					...
					exam = DynamicExam(result[0], cop_x, cop_y, date, pat_cod, result[4])
					exams.append(exam)
		except:
			print ("Error!")
		finally:
		...
		try:
			...
			if results:
				...
				for result in results:
					...
					cop_y = Utils.str_to_cop(result[2])
					date = Utils.str_to_datetime(result[3])
					exam = DynamicExam(result[0], cop_x, cop_y, date, result[4], result[5])
					exams.append(exam)
		except:
			print ("Error!")
		finally:
