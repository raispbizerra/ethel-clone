...
def p_res(readings, sensor, calibrations, escala, peso, signal, max=False):
	...
	if max:
		...
		cal = (escala / res_weight) * (readings[sensor] - calibrations[sensor][signal-1]) + calibrations[sensor][signal-1]
	else:
		cal = (escala / (peso - res_weight)) * (readings[sensor] - calibrations[sensor][signal-1]) + calibrations[sensor][signal-1]
