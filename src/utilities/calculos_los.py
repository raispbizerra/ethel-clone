...
def computes_metrics(cop_x, cop_y, height, amplitude):
    ...
    positions[REAR_L] = (np.negative(coord_ellipsis),
                         np.negative(coord_ellipsis))
    ...
    for i in range(8):
        # f = zero[i]
        _, metrics['reaction_time'][i] = reaction_time(
            cop_x[i], cop_y[i], amplitude)
        # j = metrics['i'][i]
