...
CENTER = (0.0, 0.0)  # Center of the screen
DT = 0.04  # Period
...
def belongs_to_ellipsis(cop_x, cop_y, a, b, r, x0=0.0, y0=0.0):
    if cop_y > 0.0:
...
def distance_point_line(a: tuple, b: tuple, p: tuple):
    ...
    numerator = abs(
        (a[Y] - b[Y]) * p[X] + (b[X] - a[X]) * p[Y] + (a[X] * b[Y] - b[X] * a[Y])
    )
...
def get_zeros(cop_x, cop_y):
    ...
    for i in range(8):
        for j in range(LOS_SAMPLE):
            if cop_x[i][j] == 0.0 and cop_y[i][j] == 0.0:
                zero[i] = j
                break
    return zero
...
def computes_metrics(cop_x, cop_y, height, amplitude):
    ...
    for i in range(8):
        ...
        metrics["maximum_excursion"][i] = maximum_excursions(
            cop_x[i], cop_y[i], positions[i]
        )
        # metrics['directional_control'][i] = directional_control(CENTER, positions[i], cop_x[i][:f], cop_y[i][:f])
        metrics["directional_control"][i] = directional_control(
            sp[i], cop_x[i], cop_y[i]
        )
