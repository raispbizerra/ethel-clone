...
class Handler():
    ...
    def on_show(self, window):
        ...
        for device in self.device_list:
            self.window.combo_box.insert(
                device.cod,  str(device.cod), device.name)
    ...
    def get_calibrations(self):
        ...
        '''
        cal_date, calibrations = self.device_dao.read_device_calibrations(
            self.window.app.device.cod)
        if calibrations:
            ...
            self.device_dao.create_device_calibrations(
                self.window.app.device.cod, self.window.app.device.calibrations, self.window.app.device.calibration_date)

        print(self.window.app.device.calibrations,
