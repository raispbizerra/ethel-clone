...
class Handler():
	...
	def redefine(self):
		if TESTING:
			...
			self.window.app.calibration = {'right_top': np.zeros(3), 'right_bottom':np.zeros(3), 'left_top':np.zeros(3), 'left_bottom':np.zeros(3)}

		# Images
		self.images = ('test_pontos_rt.png', 'test_pontos_rb.png', 'test_pontos_lt.png', 'test_pontos_lb.png')
		# Sensors
		...
		self.clicked_signal = self.window.calibration_button.connect('clicked', self.on_start_calibration)
	...
	def on_start_calibration(self, button):
		...
		# Set label text
		self.window.calibration_label.set_text('Posicione o dispositivo sem nenhum peso')
		# Set button label
		self.window.calibration_button.set_label('Medir')
		# Disconnect, get and connect signal
		self.window.calibration_button.disconnect(self.clicked_signal)
		self.clicked_signal = self.window.calibration_button.connect('clicked', self.on_gauge_min)
	...
	def gauge_minimum(self):
		if self.i < MIN_REP:
			...
			for j, sensor in enumerate(self.window.app.calibration.keys()):
				self.window.app.calibration[sensor] = np.array([saida[j], medium[j], maximum[j]])

			# Reset calibre
			self.calibre = np.zeros((4, MED_REP, 4))
			self.signal = np.zeros(4)
			self.readings = [{'right_top': 0, 'right_bottom': 0, 'left_top': 0, 'left_bottom': 0}, {'right_top': 0, 'right_bottom': 0, 'left_top': 0, 'left_bottom': 0}, {'right_top': 0, 'right_bottom': 0, 'left_top': 0, 'left_bottom': 0}, {'right_top': 0, 'right_bottom': 0, 'left_top': 0, 'left_bottom': 0}]

			# Show partial calibration
			self.window.new_calibration_date_label.set_text('CALIBRAÇÃO PARCIAL - MÍNIMOS')
			self.window.new_calibration_label.set_text(str(self.window.app.calibration))

			# Disconnect, get and connect signal
			self.window.calibration_button.disconnect(self.clicked_signal)
			self.clicked_signal = self.window.calibration_button.connect('clicked', self.on_gauge_med)

			# Change label and image
			self.window.calibration_label.set_text('Posicione o peso de {}kg no sensor {}'.format(self.weights[self.current_weight], self.sensors[self.current_sensor]))
			self.window.calibration_image.set_from_file('media/{}'.format(self.images[self.current_sensor]))
			self.window.calibration_button.set_sensitive(True)
			return False
	...
	def gauge_medium(self):
		if self.i < MED_REP:
			...
			if not self.current_sensor:
				...
				for i in range(100):
					for j, sensor in enumerate(self.window.app.calibration.keys()):
						wbb.p_res(self.readings[j], sensor, self.window.app.calibration, wbb.escala_eu, int(self.weights[self.current_weight] * 100), 1)
					f.write(f"Calibração {i}: {self.window.app.calibration}")
					for sensor in self.window.app.calibration.keys():
						f.write(f"Erro relativo {sensor}: {abs((self.window.app.calibration[sensor][1] - cal[sensor][1])/cal[sensor][1]) * 100.}")
					cal = self.window.app.calibration
				...
				# Disconnect, get and connect signal
				self.window.calibration_button.disconnect(self.clicked_signal)
				self.clicked_signal = self.window.calibration_button.connect('clicked', self.on_gauge_max)
				# Reset calibre
				self.calibre = np.zeros((4, MAX_REP, 4))
				self.window.new_calibration_label.set_text(str(self.window.app.calibration))
				self.window.new_calibration_date_label.set_text('CALIBRAÇÃO PARCIAL - MÍNIMOS E MÉDIOS')
			# Change label and image
			self.window.calibration_label.set_text('Posicione o peso de {}kg no sensor {}'.format(self.weights[self.current_weight], self.sensors[self.current_sensor]))
			self.window.calibration_image.set_from_file('media/{}'.format(self.images[self.current_sensor]))
			self.window.calibration_button.set_sensitive(True)
	...
	def gauge_maximum(self):
		if self.i < MAX_REP:
			...
			if not self.current_sensor:
				for j, sensor in enumerate(self.window.app.calibration.keys()):
					wbb.p_res(self.readings[j], sensor, self.window.app.calibration, wbb.escala_eu, int(self.weights[self.current_weight] * 100), 2, max=True)
				...
				self.window.calibration_button.disconnect(self.clicked_signal)
				self.clicked_signal = self.window.calibration_button.connect('clicked', self.on_save)
				self.window.new_calibration_label.set_text(str(self.window.app.calibration))
	...
	def verify_connection(self):
		if self.window.app.connection_flags['device']:
			...
			# Show status
			self.window.calibration_label.set_text('Perda de conexão com o dispositivo, tente novamente')
			# Set button label
			self.window.calibration_button.set_label('Conectar')
			# Disconnect, get and connect signal
			self.window.calibration_button.disconnect(self.clicked_signal)
			self.clicked_signal = self.window.calibration_button.connect('clicked', self.on_connect_device)
			self.window.calibration_button.set_sensitive(True)
			return False
