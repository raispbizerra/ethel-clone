...
class Handler:
    ...
    def get_exam(self):
        ...
        for k in range(1, 4):
            context = (
                self.window.exam_grid.get_child_at(k, i + 2)
                .get_child_at(j, 0)
                .get_style_context()
            )
    ...
    def nullify_device(self):
        ...
        self.window.connection_image.set_from_file("media/bluetooth_disconnect.png")
    ...
    def show_static_exam(self):
        ...
        try:
            ...
            metrics = [
                self.static_metrics["AP_"],
                self.static_metrics["ML_"],
                self.static_metrics["dis_mediaAP"],
                self.static_metrics["dis_mediaML"],
                self.static_metrics["dis_media"],
                self.static_metrics["dis_rms_AP"],
                self.static_metrics["dis_rms_ML"],
                self.static_metrics["dis_rms_total"],
                self.static_metrics["mvelo_AP"],
                self.static_metrics["mvelo_ML"],
                self.static_metrics["mvelo_total"],
                self.static_metrics["amplitude_AP"],
                self.static_metrics["amplitude_ML"],
                self.static_metrics["amplitude_total"],
                768,
            ]
            ...
            for x in range(1, ln):
                self.window.metrics_grid.get_child_at(1, x).set_text(
                    f"{round(metrics[x - 1], 2)}"
                )

            self.axis_0.set_xlim(
                -self.static_metrics["max_absoluto"],
                self.static_metrics["max_absoluto"],
            )
            self.axis_0.set_ylim(
                -self.static_metrics["max_absoluto"],
                self.static_metrics["max_absoluto"],
            )
            self.axis_0.plot(
                self.static_metrics["MLs_Processado"],
                self.static_metrics["APs_Processado"],
                ".-",
                label="CoP",
            )
            self.axis_0.legend()
            ...
            self.axis_1.set_ylim(
                -self.static_metrics["maximo"], self.static_metrics["maximo"]
            )
            self.axis_1.set_xlim(
                self.static_metrics["tempo"][0], self.static_metrics["tempo"][-1]
            )
            self.axis_1.plot(
                self.static_metrics["tempo"],
                self.static_metrics["APs_Processado"],
                "-",
                label="APs",
            )
            ...
            self.axis_1.plot(
                self.static_metrics["tempo"],
                self.static_metrics["dis_resultante_total"],
                ":",
                label="DRT",
            )
            self.axis_1.legend()
    ...
    def show_dynamic_exam(self):
        ...
        angles = np.radians(np.arange(0.0, 405.0, 45.0))
        # angles = np.radians(np.array([315., 0., 45., 90., 135., 180., 225., 270., 315.]))

        maximum_excursion = np.append(
            self.dynamic_metrics["maximum_excursion"],
            self.dynamic_metrics["maximum_excursion"][0],
        )
    ...
    def get_amplitude(self):
        ...
        if len(exams) < 3:
            ...
            self.window.app.amplitude = (
                self.static_metrics["amplitude_AP"] * 2 / 3,
    ...
    def on_load_dynamic_exam_button_clicked(self, button):
        ...
        self.clear_dynamic_chart()
        self.get_amplitude()
        self.dynamic_metrics = calc_los.computes_metrics(
    ...
    def static_posturography(self):
        ...
        if i <= STATIC_SAMPLE:
            ...
            self.window.app.static_exam = StaticExam(
                aps=self.static_cop_y,
                mls=self.static_cop_x,
                date=date,
                pat_cod=self.window.app.patient.cod,
                state=ex_type,
            )
            self.static_metrics = calc.computes_metrics(
    ...
    def counter(self):
        if self.window.app.exam_window.time > 0:
            ...
            (
                self.window.app.exam_window.r,
                self.window.app.exam_window.g,
                self.window.app.exam_window.b,
            ) = (0.3, 0.73, 0.09)
    ...
    def fill_grid(self):
        ...
        context = (
            self.window.exam_grid.get_child_at(1, i + 2)
            .get_child_at(j, 0)
            .get_style_context()
        )
        self.current_exam_labels.append(context)
        ...
        m /= j + 1
        self.window.exam_grid.get_child_at(1, i + 2).get_child_at(3, 0).set_text(
            f"{round(m, 2)}"
        )

        context = (
            self.window.exam_grid.get_child_at(2, i + 2)
            .get_child_at(j, 0)
            .get_style_context()
        )
        self.current_exam_labels.append(context)
        ...
        m /= j + 1
        self.window.exam_grid.get_child_at(2, i + 2).get_child_at(3, 0).set_text(
            f"{round(m, 2)}"
        )

        context = (
            self.window.exam_grid.get_child_at(3, i + 2)
            .get_child_at(j, 0)
            .get_style_context()
        )
        self.current_exam_labels.append(context)
        ...
        m /= j + 1
        self.window.exam_grid.get_child_at(3, i + 2).get_child_at(3, 0).set_text(
            f"{round(m, 2)}"
        )
    ...
    def on_save_static_exam_button_clicked(self, button):
        ...
        exam = self.exams[self.window.app.static_exam.state][
            self.exam_counter[self.window.app.static_exam.state]
        ]
        last_exam = self.exams[self.window.app.static_exam.state][
            self.exam_counter[self.window.app.static_exam.state] - 1
        ]
        if exam:
            print(exam.date)
            ...
            print("\n\nsave\n")
        self.exams[self.window.app.static_exam.state][
            self.exam_counter[self.window.app.static_exam.state]
    ...
    def verify_connection(self):
        ...
        if self.window.app.wiimote:
            try:
                ...
                battery = (
                    self.window.app.wiimote.state["battery"] * 100
                ) // BATTERY_MAX
